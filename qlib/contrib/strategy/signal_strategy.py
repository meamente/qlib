...
class TruncatedTopkDropoutStrategy(BaseSignalStrategy):
    # TODO:
    # 1. Supporting leverage the get_range_limit result from the decision
    # 2. Supporting alter_outer_trade_decision
    # 3. Supporting checking the availability of trade decision
    ...
    def generate_trade_decision(self, execute_result=None):
        ...
        for code in buy:
            ...
            buy_amount = value / buy_price
            factor = self.trade_exchange.get_factor(stock_id=code, start_time=trade_start_time, end_time=trade_end_time)
            buy_amount = int(self.trade_exchange.round_amount_by_trade_unit(buy_amount, factor))
            buy_order = Order(
                stock_id=code,
                amount=buy_amount,
                start_time=trade_start_time,
...
class TruncatedTopkDropoutWStatesStrategy(BaseSignalStrategy):
    # TODO:
    # 1. Supporting leverage the get_range_limit result from the decision
    # 2. Supporting alter_outer_trade_decision
    # 3. Supporting checking the availability of trade decision
    ...
    def generate_trade_decision(self, execute_result=None):
        ...
        # Get the stock list we really want to buy
        buy = today[: len(sell) + self.topk - len(last)]
        if trade_start_time in self.market_states.index:
            if self.market_states.loc[trade_start_time].item() == 0.0 and self.market_states.loc[trade_start_time - BDay(2): trade_start_time].sum().item() == 0.0:
                buy = [] # prohibit buying stocks in bear markets
                sell = last.copy()
                print(sell)
                print("buying prohibited!")
        for code in current_stock_list:
            if not self.trade_exchange.is_stock_tradable(
                stock_id=code,
                start_time=trade_start_time,
...
class TopkDropoutRiskAdjustedStrategy(BaseSignalStrategy):
    # TODO:
    # 1. Supporting leverage the get_range_limit result from the decision
    # 2. Supporting alter_outer_trade_decision
    # 3. Supporting checking the availability of trade decision
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.method_sell = method_sell
        self.method_buy = method_buy
        self.riskmodel_path = riskmodel_path
        self.hold_thresh = hold_thresh
        self.only_tradable = only_tradable
        self.forbid_all_trade_at_limit = forbid_all_trade_at_limit
    ...
    def generate_trade_decision(self, execute_result=None):
        ...
        # note the current has been changed
        # current_stock_list = current_temp.get_stock_list()
        def buy_weights(pred_time, buy_stocks):
            risk_file_path = os.path.join(self.riskmodel_path, pred_time.strftime('%Y%m%d'), 'risk.pkl')
            risk_data = np.load(risk_file_path, allow_pickle=True)
            risk_data = risk_data.to_dict()
            weights = {key: (1 / risk_data[key]) for key in buy_stocks}
            norm_factor = sum(list(weights.values()))
            for key in weights:
        ...
        stock_weightings = buy_weights(pred_start_time, buy)
        # open_cost should be considered in the real trading environment, while the backtest in evaluate.py does not
        # consider it as the aim of demo is to accomplish same strategy as evaluate.py, so comment out this line
        # value = value / (1+self.trade_exchange.open_cost) # set open_cost limit
        for code in buy:
            ...
            value = cash * self.risk_degree * stock_weightings[code]
            buy_amount = value / buy_price
            factor = self.trade_exchange.get_factor(stock_id=code, start_time=trade_start_time, end_time=trade_end_time)
            buy_amount = self.trade_exchange.round_amount_by_trade_unit(buy_amount, factor)
            buy_order = Order(
...
class TopkDropoutMktCapStrategy(BaseSignalStrategy):
    # TODO:
    # 1. Supporting leverage the get_range_limit result from the decision
    # 2. Supporting alter_outer_trade_decision
    # 3. Supporting checking the availability of trade decision
    ...
    def generate_trade_decision(self, execute_result=None):
        ...
        # note the current has been changed
        # current_stock_list = current_temp.get_stock_list()
        def buy_weights(buy_stocks):
            unif_stocks, mkt_w_stocks = [], []
            for key in buy_stocks:
                if key in self.mkt_cap_data.index:
                    mkt_w_stocks.append(key)
                else:
                    unif_stocks.append(key)
            total_cap = self.mkt_cap_data.loc[mkt_w_stocks]["mkt_cap"].sum()
            weights = {key: ((len(buy_stocks) - len(unif_stocks)) / len(buy_stocks)) * self.mkt_cap_data.loc[key]["mkt_cap"] / total_cap for key in mkt_w_stocks}
            for key in unif_stocks:
                weights[key] = 1. / len(buy_stocks)
            return weights
        
        
        stock_weightings = buy_weights(buy)
        # open_cost should be considered in the real trading environment, while the backtest in evaluate.py does not
        # consider it as the aim of demo is to accomplish same strategy as evaluate.py, so comment out this line
        # value = value / (1+self.trade_exchange.open_cost) # set open_cost limit
        for code in buy:
